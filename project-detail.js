...
const detailNarratives = {
  ...
  "nutrition-label-diet-choices": {
    ...
    body: `
      ...
      <p>That regression captures the core logic. Here \\(Y_i\\) is the downstream choice outcome for respondent \\(i\\), \\(\\alpha\\) is the intercept, \\(\\mathrm{Educated}_i\\) indicates whether that respondent received the nutrition-label intervention, \\(X_i\\) is the vector of demographic and baseline-preference controls, \\(\\beta\\) is the associated coefficient vector, and \\(\\varepsilon_i\\) is the residual term. The coefficient \\(\\tau\\) is the treatment effect of nutrition-label education once those controls are held constant. I like this formulation because it keeps the analysis honest: the intervention only matters if it moves the decision outcome after controlling for who the respondent is.</p>
