  ...
  projects: [
    {
      ...
      detailSections: {
        ...
        mathematicalCore:
          "<p>The estimand is the average treatment effect on treated counties:</p><div class=\"math-block\">\\[ \\mathrm{ATT} = \\mathbb{E}\\left[Y_{it}(1) - Y_{it}(0) \\mid D_i^{\\text{treatment}} = 1,\\; t \\geq \\text{Jan 2021}\\right]. \\]</div><p>Here, \\(Y_{it}(1)\\) and \\(Y_{it}(0)\\) are the treated and untreated potential outcomes for county \\(i\\) in month \\(t\\), and the conditioning event picks out treated California counties in the post-policy period. The core regression is a DID with fixed effects and dynamic persistence control:</p><div class=\"math-block\">\\[ \\log(Y_{it}) = \\beta_0 + \\beta_1 D_i^{\\text{treatment}} + \\beta_2 D_t^{\\text{post}} + \\beta_3 \\left(D_i^{\\text{treatment}} D_t^{\\text{post}}\\right) + \\rho \\log(Y_{i,t-1}) + \\gamma_i + \\delta_t + u_{it}. \\]</div><p>In this regression, \\(Y_{it}\\) is unemployment in county \\(i\\) during month \\(t\\); \\(D_i^{\\text{treatment}}\\) marks treated California counties; \\(D_t^{\\text{post}}\\) marks post-January-2021 months; \\(\\beta_3\\) is the policy effect of interest; \\(\\rho\\) captures persistence through lagged unemployment; \\(\\gamma_i\\) and \\(\\delta_t\\) are county and month fixed effects; and \\(u_{it}\\) is the residual shock.</p>",
        evaluation:
    ...
    },
    {
      ...
      detailSections: {
        ...
        mathematicalCore:
          "<p>The baseline regression uses time trend, seasonality, lag structure, and exogenous covariates:</p><div class=\"math-block\">\\[ Y_t = \\beta_0 + \\beta_1 t + \\sum_{m=1}^{11} \\gamma_m D_m + \\delta_1 Y_{t-1} + \\sum_{k=1}^{21} \\theta_k X_{k,t} + \\varepsilon_t. \\]</div><p>Here, \\(Y_t\\) is the rent index at month \\(t\\), \\(D_m\\) is the dummy for calendar month \\(m\\), \\(X_{k,t}\\) is the \\(k\\)-th exogenous feature at time \\(t\\), and \\(\\varepsilon_t\\) is the residual. The final winning model is Prophet, which decomposes the series additively:</p><div class=\"math-block\">\\[ y(t) = g(t) + s(t) + h(t) + \\varepsilon_t, \\]</div><p>In that decomposition, \\(y(t)\\) is the observed rent index, \\(g(t)\\) captures piecewise-linear trend, \\(s(t)\\) captures seasonality through a Fourier expansion, \\(h(t)\\) accounts for irregular recurring effects, and \\(\\varepsilon_t\\) is the remaining noise. In this project, Prophet's changepoint prior scale was tuned to 0.001 and the seasonality prior scale to 1.0 to balance flexibility against overfitting.</p>",
        evaluation:
    ...
    },
    {
      ...
      detailSections: {
        ...
        mathematicalCore:
          "<p>The retrieval score uses Reciprocal Rank Fusion across dense and sparse search:</p><div class=\"math-block\">\\[ \\mathrm{RRF}(d) = \\sum_{r \\in \\{\\text{vector},\\,\\text{BM25}\\}} \\frac{w_r}{k + \\mathrm{rank}_r(d)}. \\]</div><p>In the retrieval formula, \\(d\\) is a candidate customer document, \\(r\\) indexes the retrieval channel, \\(w_r\\) is the weight assigned to that channel, \\(k\\) is the smoothing constant, and \\(\\mathrm{rank}_r(d)\\) is the rank of document \\(d\\) under retriever \\(r\\). The fine-tuning objective is standard supervised causal language modeling:</p><div class=\"math-block\">\\[ \\mathcal{L}(\\theta; x, y) = -\\frac{1}{T} \\sum_{t=1}^{T} \\log P_\\theta(y_t \\mid y_{&lt;t}, x). \\]</div><p>In the loss, \\(\\theta\\) denotes trainable model parameters, \\(x\\) is the retrieved prompt context, \\(y\\) is the target structured response, \\(T\\) is the output length, and \\(P_\\theta(y_t \\mid y_{&lt;t}, x)\\) is the token probability at step \\(t\\). At inference time, the final risk score is computed deterministically from churn rate, tenure, charges, and contract type:</p><div class=\"math-block\">\\[ R_{\\mathrm{total}} = 0.35R_{\\mathrm{churn}} + 0.25R_{\\mathrm{tenure}} + 0.20R_{\\mathrm{charge}} + 0.20R_{\\mathrm{contract}}. \\]</div><p>Here, \\(R_{\\mathrm{churn}}\\), \\(R_{\\mathrm{tenure}}\\), \\(R_{\\mathrm{charge}}\\), and \\(R_{\\mathrm{contract}}\\) are normalized component risks, and the weights are fixed to sum to one. This combination is what makes the system both expressive and auditable: the LLM writes the narrative, but critical trust-sensitive pieces are still grounded in explicit algorithms.</p>",
        evaluation:
...
const categoryTranslationsZh = {
  ...
  "nlp-large-language-model": {
    title: "自然语言处理与大模型",
    description: "涵盖情感分析、检索增强生成、工具调用，以及基于大模型的推理系统等语言相关项目。",
  },
...
const projectTranslationsZh = {
  ...
  "email-funding-conversion-experiment": {
    ...
    primaryCategory: "因果推断与实验设计",
    coverLabel: "入金实验",
    miniDescription: "设计并分析多臂邮件实验，从实验设计、漏斗分析和产品指标角度理解用户的入金转化行为。",
    status: "已完成",
  ...
  "llm-powered-churn-analysis-system": {
    ...
    coverLabel: "流失分析",
    miniDescription: "结合 hybrid retrieval RAG、模型微调和结构化 JSON 输出，构建自然语言流失分析系统。",
    status: "已完成",
